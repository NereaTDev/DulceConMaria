...
@section('title', 'Nueva inscripción · Admin')

@section('content')
    <h1 class="text-2xl font-semibold mb-6">Nueva inscripción</h1>

    <form action="{{ route('admin.enrollments.store') }}" method="POST" class="space-y-6 max-w-2xl text-sm">
        @csrf
        ...
        <div>
            ...
            <select name="user_id" class="w-full border rounded px-3 py-2 text-sm" required>
                @foreach($users as $userOption)
                    <option value="{{ $userOption->id }}" @selected(old('user_id') == $userOption->id)>
        ...
        <div>
            ...
            <select name="course_id" class="w-full border rounded px-3 py-2 text-sm" required>
                @foreach($courses as $courseOption)
                    <option value="{{ $courseOption->id }}" @selected(old('course_id') == $courseOption->id)>
                        {{ $courseOption->title }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="flex gap-3">
            <button type="submit" class="inline-flex items-center rounded-md bg-pink-500 px-4 py-2 text-sm font-semibold text-white hover:bg-pink-600">Guardar inscripción</button>
