...
@section('content')
    ...
    <form action="{{ route('admin.recipes.update', $recipe) }}" method="POST" enctype="multipart/form-data" class="space-y-6 max-w-2xl">
        ...
        <div>
            ...
            <select name="course_id" class="w-full border rounded px-3 py-2 text-sm">
                ...
                @foreach ($courses as $course)
                    <option value="{{ $course->id }}" @selected(old('course_id', $recipe->course_id) == $course->id)>{{ $course->title }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $recipe->title) }}" class="w-full border rounded px-3 py-2 text-sm" required>
        </div>

        <div>
            <label class="block text-sm font-medium mb-1">Ingredientes (uno por línea)</label>
            <textarea name="ingredients" rows="6" class="w-full border rounded px-3 py-2 text-sm" required>{{ old('ingredients', $ingredientsText) }}</textarea>
        </div>

        <div>
            <label class="block text-sm font-medium mb-1">Preparación</label>
            <textarea name="description" rows="8" class="w-full border rounded px-3 py-2 text-sm" required>{{ old('description', $recipe->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            <select name="lesson_ids[]" multiple class="w-full border rounded px-3 py-2 text-sm h-32">
                @php
                    $selectedLessons = collect(old('lesson_ids', $recipe->lessons->pluck('id') ?? []));
                @endphp
                @foreach ($lessons as $lesson)
                    <option value="{{ $lesson->id }}" @selected($selectedLessons->contains($lesson->id))>
        ...
        <div class="flex items-center gap-2">
            <input type="checkbox" name="is_public" id="is_public" class="border rounded" @checked(old('is_public', $recipe->is_public))>
